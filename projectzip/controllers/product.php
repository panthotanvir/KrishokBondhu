...
class Product extends CI_Controller{
		...
		{	
			if(isset($_POST['upload'])){
				...
				$this->form_validation->set_rules('year', 'Year', 'required|numeric|exact_length[4]|greater_than[2010]');
				$this->form_validation->set_rules('month', 'Month', 'required|numeric|exact_length[2]|greater_than[0]|less_than[13]');
				$this->form_validation->set_rules('date', 'Date', 'required|numeric|exact_length[2]|greater_than[0]|less_than[32]');
				...
				//echo $amount;
				$price= $_POST['price'];
				$production_date= $_POST['year']."/".$_POST['month']."/".$_POST['date'];
		...
		//echo $user_nid;
				...
				if ( ! $this->upload->do_upload())
				{
					...
					$this->load->view('template/header',$error);
							$this->load->view('template/menubar',$error);
							$this->load->view('template/sidebar',$error);
							$this->load->view('add_customer_page',$error);
							$this->load->view('template/footer',$error);
				}
				else
				{
				...
				//echo $user_nid;
					...
					$image = $this->dbmodel->addSell($data,$user_id,$title,$amount,$price,$production_date,$description,$location);
					$data1= array('user_id'=> $user_id);
					$this->load->view('template/header',$data1);
							$this->load->view('template/menubar',$data1);
		...
		public function buyPostConfirm(){
			if(isset($_POST['upload'])){
				...
				//echo $amount;
				$price= $_POST['price'];
				$required_date= $_POST['year']."/".$_POST['month']."/".$_POST['date'];;
				$description= $_POST['description'];
				$location= $_POST['location'];
		//echo $user_nid;
					$image = $this->dbmodel->addBuy($user_id,$title,$amount,$price,$required_date,$description,$location);
					$data1= array('user_id'=> $user_id);
					$this->load->view('template/header',$data1);
					$this->load->view('template/menubar',$data1);
					$this->load->view('template/sidebar',$data1);
		...
		public function marketAnalysis(){
			...
			$this->load->view('template/menubar',$data);
			$this->load->view('template/sidebar',$data);
			$this->load->view('market_selection',$data);
			$this->load->view('template/footer',$data);
		}
		public function marketAnalysisPost(){
			if(isset($_POST['enter'])){
				...
				if ($this->form_validation->run() == FALSE)
				{
					...
					$result = $this->dbmodel->marketAnalysisDB($product_district);
